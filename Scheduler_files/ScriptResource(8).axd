...
}if(!this._onMouseUpDelegate){this._onMouseUpDelegate=Function.createDelegate(this,this._onMouseUpHandler);
...
if(this._canDragDrop&&((this._data.Reorderable&&this._owner._owner.ClientSettings.AllowColumnsReorder)||(this._data.Groupable&&this._owner._owner.ClientSettings.AllowDragToGroup))){Telerik.Web.UI.Grid.CreateDragDrop(d,this,true);
}}if(this._canResize&&(d.button==0)){var c=Telerik.Web.UI.Grid.GetEventPosX(d);
var b=Telerik.Web.UI.Grid.FindPosX(this.get_element());
var a=b+this.get_element().offsetWidth;
if((c>=a-this._resizeTolerance)&&(c<=a+this._resizeTolerance)){this._columnResizer=new Telerik.Web.UI.GridColumnResizer(this,this._owner._owner.ClientSettings.Resizing.EnableRealTimeResize);
this._columnResizer._position(d);
}Telerik.Web.UI.Grid.ClearDocumentEvents();
}},_onMouseUpHandler:function(f){if(this._onMouseUpDelegate){$telerik.removeExternalHandler(document,"mouseup",this._onMouseUpDelegate);
this._onMouseUpDelegate=null;
...
if(g!=null&&this._canDragDrop&&!this._owner._owner._isResize){var c=this._owner._owner.ClientSettings.PostBackFunction;
...
}}if(this._owner._owner.ClientSettings.AllowColumnsReorder&&Telerik.Web.UI.Grid.IsChildOf(g,this.get_element().parentNode)&&g!=this.get_element()){var b=g.parentNode;
while(b!=null&&(typeof(b.UniqueName)=="undefined")){b=b.parentNode;
}if(b!=null&&typeof(b.UniqueName)!="undefined"&&b.UniqueName!=this.get_uniqueName()){g=b;
}if(typeof(g.UniqueName)!="undefined"&&this._canDropOnThisColumn(g.UniqueName)&&this.get_reorderable()){if(!this._owner._owner.ClientSettings.ReorderColumnsOnClient){var a=this._owner.getColumnByUniqueName(this.get_element().UniqueName);
var d=this._owner.getColumnByUniqueName(g.UniqueName);
var h=new Sys.CancelEventArgs();
h.get_gridSourceColumn=function(){return a;
};
h.get_gridTargetColumn=function(){return d;
};
this._owner._owner.raise_columnSwapping(h);
if(h.get_cancel()){return false;
}c=c.replace("{1}","ReorderColumns,"+this._owner._data.UniqueID+","+this.get_element().UniqueName+","+g.UniqueName);
eval(c);
...
if(window.netscape){this._moveHeaderDiv.className+=" "+d._owner._owner.get_element().className;
...
Telerik.Web.UI.Grid.MoveDragDrop=function(f,a,b){if(this._moveHeaderDiv!=null){if(typeof(this._moveHeaderDiv.style.filter)!="undefined"){this._moveHeaderDiv.style.filter="alpha(opacity=25);";
...
},_click:function(j){var h=(j.target)?j.target:j.srcElement;
if(!h.tagName){return;
}if(h.tagName.toLowerCase()=="label"&&h.htmlFor){return;
}if(!this._shouldRaiseRowEvent(h)){return;
}var k=(h.tagName.toLowerCase()=="input"&&h.type.toLowerCase()=="checkbox"&&(h.id&&h.id.indexOf("SelectCheckBox")!=-1));
if(this._owner.ClientSettings.Selecting&&this._owner.ClientSettings.Selecting.AllowRowSelect){if(h.tagName.toLowerCase()!="tr"){h=Telerik.Web.UI.Grid.GetFirstParentByTagName(h,"tr");
}if(this._owner.ClientSettings.Selecting.UseClientSelectColumnOnly&&!k&&this._findClientSelectColumn(h)){}else{var l=h;
var g=false;
while(h&&Telerik.Web.UI.Grid.IsChildOf(h,this._owner.get_element())){if(h.id&&h.id.split("__").length==2){g=true;
break;
}h=Telerik.Web.UI.Grid.GetFirstParentByTagName(h.parentNode,"tr");
}if(!g){h=l;
}if(h&&(h.parentNode.parentNode.parentNode==this._owner.get_element()||h.parentNode.parentNode.parentNode==this._owner._gridDataDiv||Array.contains(this._owner.get_detailTables(),$find(h.parentNode.parentNode.id)))&&h.id&&h.id.split("__").length==2){if(this._owner.get_allowMultiRowSelection()){if(j.shiftKey&&this._owner._selectedItemsInternal[0]){var c=$get(this._owner._selectedItemsInternal[0].id);
if(c){if(c.rowIndex>h.rowIndex){for(var a=h.rowIndex;
a<c.rowIndex+1;
a++){var b=c.parentNode.parentNode.rows[a];
if(b.id){this._selectRowInternal(b,j,true,false,true);
}}}if(c.rowIndex<h.rowIndex){for(var a=c.rowIndex;
a<h.rowIndex+1;
a++){var b=c.parentNode.parentNode.rows[a];
if(b.id){this._selectRowInternal(b,j,true,false,true);
}}}}return;
}this._selectRowInternal(h,j,k,true,true);
}else{this._selectRowInternal(h,j,false,false,true);
}}}}if(this._owner.ClientSettings&&this._owner.ClientSettings.EnablePostBackOnRowClick&&h){if(h&&h.tagName.toLowerCase()!="tr"){h=Telerik.Web.UI.Grid.GetFirstParentByTagName(h,"tr");
}if(h&&h.id!=""&&h.id.split("__").length==2){var d=h.id.split("__")[1];
var f=this._owner.ClientSettings.PostBackFunction;
f=f.replace("{0}",this._owner.UniqueID);
f=f.replace("{1}","RowClick;"+d);
...
if(b&&b.rows){for(var e=0,c=b.rows.length;
e<c;
e++){if(b.rows[e].cells&&b.rows[e].cells[0]){if(d>b.rows[e].cells.length){b.rows[e].cells[0].colSpan=d;
}}}}if(a&&a.rows){for(var e=0,c=a.rows.length;
e<c;
e++){if(a.rows[e]&&(a.rows[e]==this.get_element().HeaderRow||a.rows[e].cells[0].tagName.toLowerCase()=="th")){break;
}if(a.rows[e]&&a.rows[e].cells&&a.rows[e].cells.length>0&&a.rows[e].cells[0]){a.rows[e].cells[0].colSpan=d;
}}}},_getVisibleColumns:function(){var b=[];
if(this.get_columns()){var d=this.get_columns();
for(var e=0,c=d.length;
e<c;
e++){var a=d[e];
if(a.get_element().style.visibility!="hidden"&&(a.Display==null||a.Display)){Array.add(b,a);
}}}return b;
...
}},createItems:function(f){this._onMenuElementClickDelegate=Function.createDelegate(this,this._menuElementClick);
...
for(var b=0;
b<f.length;
b++){var a=this.get_element().insertRow(-1);
a.insertCell(-1);
...
c.cellSpacing="0";
c.insertRow(-1);
var d=c.rows[0].insertCell(-1);
var e=c.rows[0].insertCell(-1);
if(this._owner.Skin==""){d.style.borderTop="solid 1px "+this._owner._filterMenuData.SelectColumnBackColor;
d.style.borderLeft="solid 1px "+this._owner._filterMenuData.SelectColumnBackColor;
d.style.borderRight="none 0px";
d.style.borderBottom="solid 1px "+this._owner._filterMenuData.SelectColumnBackColor;
d.style.padding="2px";
d.style.textAlign="center";
}else{Sys.UI.DomElement.addCssClass(d,"GridFilterMenuSelectColumn_"+this._owner.Skin);
}d.style.width="16px";
d.appendChild(document.createElement("img"));
d.childNodes[0].src=this._owner._filterMenuData.NotSelectedImageUrl;
if(this._owner.Skin==""){e.style.borderTop="solid 1px "+this._owner._filterMenuData.TextColumnBackColor;
e.style.borderLeft="none 0px";
e.style.borderRight="solid 1px "+this._owner._filterMenuData.TextColumnBackColor;
e.style.borderBottom="solid 1px "+this._owner._filterMenuData.TextColumnBackColor;
e.style.padding="2px";
e.style.backgroundColor=this._owner._filterMenuData.TextColumnBackColor;
e.style.cursor="pointer";
}else{Sys.UI.DomElement.addCssClass(e,"GridFilterMenuTextColumn_"+this._owner.Skin);
}e.innerHTML=f[b].Text;
a.cells[0].appendChild(c);
a.id=f[b].UID;
var g={};
g.id=a.id;
g.Value=f[b].Value;
g.Image=d.childNodes[0];
this._items[this._items.length]=g;
}},_menuElementClick:function(f){var a=this._owner.ClientSettings.PostBackFunction;
var c=this.get_element().column._owner._data.UniqueID;
var b=Telerik.Web.UI.Grid.GetFirstParentByTagName(Telerik.Web.UI.Grid.GetCurrentElement(f),"tr");
if(b){var d=Telerik.Web.UI.Grid.GetFirstParentByTagName(b.parentNode,"tr");
if(d){a=a.replace("{0}",d.id).replace("{1}",c+"!"+this.get_element().column.get_element().UniqueName);
eval(a);
}}},_menuElementMouseover:function(f){this._removeFilterRowStyles();
var b=Telerik.Web.UI.Grid.GetFirstParentByTagName(Telerik.Web.UI.Grid.GetCurrentElement(f),"tr");
if(b){var g=Telerik.Web.UI.Grid.GetFirstParentByTagName(b.parentNode,"tr");
if(g){var c=g.cells[0].childNodes[0].rows[0].cells[0];
var d=g.cells[0].childNodes[0].rows[0].cells[1];
if(this._owner.Skin!=""){Sys.UI.DomElement.addCssClass(c,"GridFilterMenuHover_"+this._owner.Skin);
Sys.UI.DomElement.addCssClass(d,"GridFilterMenuHover_"+this._owner.Skin);
}else{var a=this._owner._filterMenuData;
c.style.backgroundColor=a.HoverBackColor;
c.style.borderTop="solid 1px "+a.HoverBorderColor;
c.style.borderLeft="solid 1px "+a.HoverBorderColor;
c.style.borderBottom="solid 1px "+a.HoverBorderColor;
d.style.backgroundColor=a.HoverBackColor;
d.style.borderTop="solid 1px "+a.HoverBorderColor;
d.style.borderRight="solid 1px "+a.HoverBorderColor;
d.style.borderBottom="solid 1px "+a.HoverBorderColor;
}this._overRow=g;
}}},_removeFilterRowStyles:function(){if(this._overRow){var a=this._overRow.cells[0].childNodes[0].rows[0].cells[0];
var c=this._overRow.cells[0].childNodes[0].rows[0].cells[1];
if(this._owner.Skin!=""){Sys.UI.DomElement.removeCssClass(a,"GridFilterMenuHover_"+this._owner.Skin);
Sys.UI.DomElement.removeCssClass(c,"GridFilterMenuHover_"+this._owner.Skin);
}else{var b=this._owner._filterMenuData;
a.style.borderTop="solid 1px "+b.SelectColumnBackColor;
a.style.borderLeft="solid 1px "+b.SelectColumnBackColor;
a.style.borderBottom="solid 1px "+b.SelectColumnBackColor;
a.style.backgroundColor="";
c.style.borderTop="solid 1px "+b.TextColumnBackColor;
c.style.borderRight="solid 1px "+b.TextColumnBackColor;
c.style.borderBottom="solid 1px "+b.TextColumnBackColor;
c.style.backgroundColor=b.TextColumnBackColor;
}}},_menuElementMouseout:function(a){this._removeFilterRowStyles();
this._overRow=null;
},show:function(a,d){this.hide();
this.showItems(a._data.FilterListOptions,a._data.DataTypeName,a._data.CurrentFilterFunction,a);
d.cancelBubble=true;
...
b.get_menu=function(){return c;
};
b.get_tableView=function(){return c._owner;
};
b.get_column=function(){return a;
};
b.get_domEvent=function(){return d;
};
this._owner.raise_filterMenuShowing(b);
if(b.get_cancel()){return;
}this.get_element().style.display="";
this.get_element().style.top=d.clientY+document.documentElement.scrollTop+document.body.scrollTop+5+"px";
this.get_element().style.left=d.clientX+document.documentElement.scrollLeft+document.body.scrollLeft+5+"px";
this.get_element().column=a;
...
}},showItems:function(f,c,e,d){for(var a=0;
a<this._items.length;
a++){var b=$get(this._items[a].id);
if(c=="System.Boolean"){if((this._items[a].Value=="GreaterThan")||(this._items[a].Value=="LessThan")||(this._items[a].Value=="GreaterThanOrEqualTo")||(this._items[a].Value=="LessThanOrEqualTo")||(this._items[a].Value=="Between")||(this._items[a].Value=="NotBetween")){b.style.display="none";
continue;
}}if(c!="System.String"){if((this._items[a].Value=="StartsWith")||(this._items[a].Value=="EndsWith")||(this._items[a].Value=="Contains")||(this._items[a].Value=="DoesNotContain")||(this._items[a].Value=="IsEmpty")||(this._items[a].Value=="NotIsEmpty")){b.style.display="none";
continue;
}}if(f==0){if(this._items[a].Value=="Custom"){b.style.display="none";
continue;
}}if((d._data.ColumnType=="GridDateTimeColumn"||d._data.ColumnType=="GridMaskedColumn"||d._data.ColumnType=="GridNumericColumn")&&((this._items[a].Value=="Between")||(this._items[a].Value=="NotBetween"))){b.style.display="none";
continue;
}if(e==a){this._items[a].Image.src=this._owner._filterMenuData.SelectedImageUrl;
}else{this._items[a].Image.src=this._owner._filterMenuData.NotSelectedImageUrl;
}b.style.display="";
}}};
...
try{$telerik.$.ajax({type:"GET",url:f,contentType:"application/json; charset=utf-8",dataFilter:function(e,k){return Sys.Serialization.JavaScriptSerializer.deserialize(e,true);
...
},_mouseOut:function(a){},_mouseDown:function(m){if(!this._canRiseRowEvent(m)||(this._isRowResize&&this._isRowResize==true)){return;
...
}if(this.get_allowMultiRowSelection()&&(m.ctrlKey||(m.rawEvent&&m.rawEvent.metaKey))){return;
}if(this._draggedItems){this._draggedItems=[];
}var a=Telerik.Web.UI.Grid.GetFirstParentByTagName(Telerik.Web.UI.Grid.GetCurrentElement(m),"tr");
if(a.id==""){return;
}var p=this.get_masterTableView()._getRowByIndexOrItemIndexHierarchical(a);
var c=p.id.split("__")[0];
var l=$find(c);
if(!l){return;
}var b=false;
for(var h=0;
h<this._selectedItemsInternal.length;
h++){if(this._selectedItemsInternal[h].id==a.id){b=true;
break;
}}if(!b){var g=this.get_allowMultiRowSelection()&&!this.ClientSettings.Selecting.EnableDragToSelectRows;
if(Telerik.Web.UI.GridSelection&&this._selection&&(g||!this.get_allowMultiRowSelection())){var o=this.ClientSettings.EnablePostBackOnRowClick;
this.ClientSettings.EnablePostBackOnRowClick=false;
this._selection._click(m);
this.ClientSettings.EnablePostBackOnRowClick=o;
}else{return;
}}this._canMoveRow=true;
this._originalDragItem=a;
var d=new Telerik.Web.UI.GridDataItemCancelEventArgs(a,m);
...
var j=[];
var f=l.get_selectedItems();
for(var h=0;
h<f.length;
h++){if(Array.contains(l.get_dataItems(),f[h])){var k=f[h].get_element();
j[j.length]=String.format("<tr class='{0}'>",k.className);
j[j.length]=k.innerHTML;
j[j.length]="</tr>";
Array.add(this._draggedItems,f[h]);
}}this._draggedRow.innerHTML=String.format("<table class='{0}'><tbody>{1}</tbody></table>",a.parentNode.parentNode.className,j.join(""));
var n=this._draggedRow.getElementsByTagName("table")[0];
if(this._draggedRow.mergeAttributes){this._draggedRow.mergeAttributes(this.get_element());
}else{Telerik.Web.UI.Grid.CopyAttributes(this._draggedRow,this.get_element());
}this._draggedRow.className+=String.format(" GridDraggedRows_{0}",this.Skin);
this._draggedRow.style.height="";
if(n.mergeAttributes){n.mergeAttributes(a.parentNode.parentNode);
}else{Telerik.Web.UI.Grid.CopyAttributes(n,a.parentNode.parentNode);
}this._draggedRow.style.zIndex=99999;
this._draggedRow.style.display="none";
this._draggedRow.style.width=this.get_element().offsetWidth+"px";
document.body.insertBefore(this._draggedRow,document.body.firstChild);
this._createDropClue();
if(!($telerik.isFirefox&&m.button==2&&navigator.userAgent.indexOf("Mac"))){Telerik.Web.UI.Grid.ClearDocumentEvents();
}return false;
...
}},_handleActiveRowNavigation:function(d){var b=d.keyCode||d.charCode;
var c=Telerik.Web.UI.Grid.GetCurrentElement(d);
if(c!=null&&c.tagName&&(c.tagName.toLowerCase()=="input"||c.tagName.toLowerCase()=="textarea")){return;
}var a=null;
if(this._activeRow){a=this._getNextActiveRow(this._activeRow,b);
}else{if(this._selectedItemsInternal.length>0){a=this._getNextActiveRow($get(this._selectedItemsInternal[this._selectedItemsInternal.length-1].id),b);
}else{a=this.get_masterTableView()._getFirstDataRow();
}}if(!a){if(!this.get_allowActiveRowCycle()){return;
}if(b==38){a=this.get_masterTableView()._getLastVisibleDataRow();
}else{if(b==40){a=this.get_masterTableView()._getFirstDataRow();
}}if(!a){return;
}}this._setActiveRow(a,d);
if(this.ClientSettings.Selecting&&this.ClientSettings.Selecting.AllowRowSelect&&!d.ctrlKey){this._selection._selectRowInternal(a,d,false,true,true,false);
}d.preventDefault();
